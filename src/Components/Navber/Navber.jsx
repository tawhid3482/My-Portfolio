...
const {user,isAdmin,logOut}=useState()
  ...
  return (
    <nav className="flex justify-between items-center md:justify-center md:items-center md:text-white p-1 bg-black">
      ...
      >
        {openMenu === true ? (
          <RxCross1 className="text-3xl text-yellow-400"></RxCross1>
        ) : (
          <MdOutlineMenuOpen className="text-3xl text-yellow-400"></MdOutlineMenuOpen>
        )}
      </div>

      <ul
        className={` md:flex  md:justify-center rounded-md w-40  md:w-full duration-1000 absolute right-3 md:static bg-black text-white p-4
